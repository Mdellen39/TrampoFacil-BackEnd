...
import { loginUsuarioDTO } from "./Dtosusuarios/LoginUsuario.dto";
import { alteraUsuarioDTO } from "./Dtosusuarios/AlterarUsuario.dto";
import { ListaUsuarioDTO } from "./Dtosusuarios/ListasUsuarios.dto";
...
@Post()
    //essa linha, seria um decorator para definir que a função é um metodo POST
    //Para receber dados do body da requisição, deve utilizar o decorator de "Body", especificando depois a variavel
    //logo ficaria assim no postman "nome" : "Matheus Mariano"
async criaUsuario(@Body() dadosUsuario: criaUsuarioDTO){       
    ...
    var novoUsuario = new UsuarioEntity(uuid(), dadosUsuario.nome, dadosUsuario.idade, 
                                        ...
                                        dadosUsuario.telefone, dadosUsuario.senha, dadosUsuario.doc
    )
       //gravação do usuário, aqui é inserido no DM o usuário criado anteriormente
       this.Usuarios.AdicionarUsuario(novoUsuario);

       //criação do padrão de retorno, para depois ser retornado como resposta do método, 
       // também é retornado os dados do usuário logado
       var retorno = new RetornoUsuarioDTO('Usuario criado',novoUsuario);        
       return retorno        
   }

       @Post('/login')
       //linha que define o método post para login, nesse caso é idenficado o URL
        ...
        @Put('/:id')
        //linha que define o método vai ser de alteração (put), 
        // nesse caso também é especificado um parametro na URL, por onde vai chegar o id do usuário
        async alteraUsuario(@Body() dadosNovos: alteraUsuarioDTO,@Param('id') id: string){
        //aqui é definido que vai receber dados tanto do body quanto da URL(param)
    ...
    @Get('/:ID')
    //criação de método GET, para retornar usuários filtrados pelo ID, onde é necessário passar o ID do usuário pelo url 
    async retornaUsuarioId(@Param('ID') ID:string){
    //aqui é feita a pesquisa do usuário, 
    //depois é criado mapeado os dados desse usuário para um retorno padrão (lista usuario DTO)
